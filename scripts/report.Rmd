...
```{r}
fname_prodigal_stats <- "ideel/prodigal_stats.tsv"
...
df.prodigal_stats <- read_tsv(fname_prodigal_stats, col_names = TRUE, col_types = "fcciiidi") %>%
    ...
    select(assembly, num_seqs, min_len, avg_len, max_len, sum_len)
...
Measured is the number of ORFs with a database match, the fraction of ORFs that have the same length or ≥95% the length of the best-matching database sequence (higher fractions being better).

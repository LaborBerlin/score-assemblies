...
cat("## Quast\n")
for(r in list_ref) {
  x <- paste0("* [Quast for ", r,"](quast/", r, "/report.html)\n")
  cat(x)
}
```
...
df.single_nuc <- df.assess_homopolymers %>%
  ...
  mutate(across(starts_with("len"), round, 4)) %>% #this rounding is necessary, because the thresholds generated by DT::styleInterval() are also rounded to 4 digits after the .
  mutate(mean = round(mean, 4))

dt <- df.single_nuc %>%
  mutate(assembly = paste0('<a href="pomoxis/', assembly, '/assess_homopolymers/', assembly, '_', reference, '_count/hp_correct_vs_len.png">', assembly, '</a>')) %>%
  DT::datatable(escape = FALSE, rownames = FALSE, class="compact hover",
  ...
  formatRound(3:9, digits = 4)

color_dt(dt, 3:9, rep(green,7), df.single_nuc)
```
...
Measured is the fraction of predicted ORFS that were ≥95% the length of their best-matching protein sequence in the database (higher fractions being better).
...
asis_output("ORFs are predicted in each assembly using Prodigal, which are then searched against the provided reference protein sequences using diamond.  \n")
asis_output("Measured is the number and fraction of predicted ORFS, which have the same length as their best-matching reference protein sequence.\n")

df.ideel_ref <- data.frame(assembly = character(), reference = character(), n_seq_ref = integer(), n_ORFs = integer(), n_unique_ssid = integer(), n_full_length_ORFs = integer(), n_min95perc_ORFs = integer())
...
for (r in seq_along(ref_list)) {
  ...
  len_faa_ref <- length(readAAStringSet(paste0("references-protein/", ref, ".faa")))
	...
	for (i in seq_along(filelist)) {
		...
		df <- read_tsv(filename, col_names = c("qlen", "slen", "sseqid"),	col_types = "iic")
  	...
  	assembly <- filelist[[i]] %>% str_remove("\\.tsv") %>% str_remove(paste0("_", ref))
	
  	df.ideel_ref <- df.ideel_ref %>% add_row(reference = ref, assembly = assembly, n_seq_ref = len_faa_ref, n_ORFs = n_total, n_unique_ssid = n_unique_ssid, n_full_length_ORFs = n_same_len, n_min95perc_ORFs = n_min95perc_len)
	}
}
...
color_dt(dt, 4:7, c(green, green, green, green), df.ideel_ref)
```

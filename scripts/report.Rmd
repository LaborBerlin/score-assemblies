...
df.ideel_ref <- data.frame(assembly = character(), reference = character(), n_seq_ref = integer(), n_unique_qsid = integer(), n_unique_ssid = integer(), n_full_length_ORFs = integer(), n_min95perc_ORFs = integer())
...
for (r in seq_along(ref_list)) {
  ...
  for (i in seq_along(filelist)) {
    ...
    df.ideel_ref <- df.ideel_ref %>% add_row(reference = ref, assembly = assembly, n_seq_ref = len_faa_ref, n_unique_qsid = n_unique_qsid, n_unique_ssid = n_unique_ssid, n_full_length_ORFs = n_same_len, n_min95perc_ORFs = n_min95perc_len)
  }
}
...
color_dt(dt, 5:7, c(green, green, green), df.ideel_ref)
```

...
asis_output("The table shows the rates of correct homopolymers of length 3 to 8 for the four bases A, C, G, and T as well as the mean of all values in the last column. NB: This mean is a bit stupid. For example, if there only two homopolymers of length 8 and one is wrong, the rate would be 0.5 for length 8 and this 0.5 will go into the mean with equal weight as the other lengths, overly drawing the mean down.\n")
...
# TODO load number of reference proteins from faa files and add to table
...
color_dt(dt, 3:5, c(green, green, green), df.ideel_ref)
```
...
# make list of reference genomes
...
dt <- df.out %>%
  DT::datatable(elementId = "dt_summary", container = sketch, colnames = list_headings, escape = FALSE, rownames = FALSE, class="compact hover", extensions = "FixedColumns",

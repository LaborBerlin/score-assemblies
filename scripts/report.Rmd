...
df.dnadiff %>%
  ...
  formatStyle(names(df.dnadiff), 'vertical-align' = 'top')
```
...
df.nucdiff %>%
     DT::datatable(escape = FALSE, rownames = FALSE, class="compact hover",
               options = list(pageLength = 10, searchHighlight = TRUE, paginate = TRUE, lengthChange = FALSE, fixedHeader = TRUE, order = list(list(4, 'desc'), list(3, 'desc'), list(2, 'desc')))) %>%
  formatStyle(names(df.nucdiff), 'font-size' = '90%') %>%
  formatStyle(names(df.nucdiff), 'vertical-align' = 'top')
```
...
for (i in 2:ncol(df.out)) {
  ...
  if(list_base_colors[[i]] == green) {
    ...
    list_breaks <- seq(second_lowest_value, max(values), length.out = 10) - 0.00001

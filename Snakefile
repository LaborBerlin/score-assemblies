...
list_ideel_ref_pdf = []
if len(references) > 0:
	list_assess_assembly_summ = expand(out_dir + "/pomoxis/{id}/assess_assembly/{id}_{ref}_summ.txt", id=assemblies, ref=references)
	list_assess_assembly_meanQ_tsv = expand(out_dir + "/pomoxis/{id}/assess_assembly/{id}_{ref}_scores.tsv", id=assemblies, ref=references)
	list_assess_assembly_meanQ_pdf = expand(out_dir + "/pomoxis/{ref}_assess_assembly_all_meanQ.pdf", ref=references)
	list_assess_homopolymers_rel_len = expand(out_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}_rel_len.tsv", id=assemblies, ref=references)
	list_assess_homopolymers_correct_len_tsv = expand(out_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}_correct_len.tsv", id=assemblies, ref=references)
	list_pomoxis = [out_dir + "/pomoxis/assess_assembly_all_scores.tsv", out_dir + "/pomoxis/assess_homopolymers_all_rel_len.tsv", out_dir + "/pomoxis/assess_homopolymers_all_correct_len.tsv" ]
	list_quast_report = expand(out_dir + "/quast/{ref}/report.html", ref=references)
	list_dnadiff_report = expand(out_dir + "/dnadiff/{ref}/{id}-dnadiff.report", id=assemblies, ref=references)
	list_dnadiff = [ out_dir + "/dnadiff/all_stats.tsv" ]
	list_dnadiff_tsv = expand(out_dir + "/dnadiff/{ref}/{id}-dnadiff-stats.tsv", id=assemblies, ref=references)
	list_dnadiff_pdf = expand(out_dir + "/dnadiff/{ref}_dnadiff_stats.pdf", ref=references)
	list_nucdiff_stat = expand(out_dir + "/nucdiff/{ref}/{id}-nucdiff/results/nucdiff_stat.out", id=assemblies, ref=references)
	list_nucdiff_tsv = expand(out_dir + "/nucdiff/{ref}/{id}-nucdiff/nucdiff.tsv", id=assemblies, ref=references)
	list_nucdiff = [ out_dir + "/nucdiff/all_stats.tsv" ]
	list_nucdiff_pdf = expand(out_dir + "/nucdiff/{ref}_nucdiff_stats.pdf", ref=references)

if len(references_protein) > 0:
	list_ideel_ref_tsv = expand(out_dir + "/ideel/diamond-ref/{ref}/{id}_{ref}.tsv", id=assemblies, ref=references_protein)
	list_ideel_ref_pdf = expand(out_dir + "/ideel/{ref}_ideel_{type}.pdf", ref=references_protein, type=['histograms', 'boxplots'])

list_busco_out = expand(out_dir + "/busco/{id}/short_summary.specific.{blin}_odb10.{id}.txt", id=assemblies, blin=busco_lineage)
list_busco_tsv = expand(out_dir + "/busco/{id}/short_summary.specific.{blin}_odb10.{id}.tsv", id=assemblies, blin=busco_lineage)
...
rule assess_assembly:
	...
	log: log_dir + "/pomoxis/{id}/assess_assembly/{id}_{ref}_log.txt"
	shell:
		"""
...
rule assess_homopolymers_minimap:
	...
	input:
		assembly = "assemblies/{id}.fa",
		ref = "references/{ref}.fa"
	output: out_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}.bam"
	log: log_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}.minimap2.log"
	shell:
		"""
...
rule assess_homopolymers:
	...
	output:
		...
		correct_len2 = out_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}_correct_len.tsv"
	params:
		count_dir = directory(out_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}_count"),
		analyse_dir = directory(out_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}_analyse")
	log: log_dir + "/pomoxis/{id}/assess_homopolymers/{id}_{ref}_log.txt"
	shell:
		"""
...
rule busco:
	...
	log: log_dir + "/busco/{id}/busco_{busco_lineage}.log"
	shell:
		"""
...
rule dnadiff:
	...
	log: log_dir + "/dnadiff/{ref}/{id}-dnadiff.log"
	shell:
		"""
...
rule nucdiff:
	...
	log: log_dir + "/nucdiff/{ref}/{id}-nucdiff.log"
	shell:
		"""
...
rule download_uniprot:
	...
	output:
		out_dir + "/ideel/uniprot/uniprot_sprot.fasta.gz"
	log: log_dir + "/ideel/uniprot/download.log"
	shell:
		"""
...
rule diamond_makedb:
	...
	output:
		out_dir + "/ideel/uniprot/uniprot_sprot.dmnd"
	log: log_dir + "/diamond-makedb-uniprot.log"
	shell:
		"""
...
rule prodigal:
	...
	output: out_dir + "/ideel/prodigal/{id}.faa"
	log: log_dir + "/ideel/prodigal/{id}.log"
	shell:
		"""
...
rule diamond:
  threads: 5
	...
	output: out_dir + "/ideel/diamond/{id}.tsv"
...
rule diamond_ref:
  threads: 5
	...
	output: out_dir + "/ideel/diamond-ref/{ref}/{id}_{ref}.tsv"
...
rule plot_assess_assembly:
	...
	input: out_dir + "/pomoxis/assess_assembly_all_scores.tsv"
	output: list_assess_assembly_meanQ_pdf
...
rule plot_busco:
	conda: "env/env-r.yaml"
	input: out_dir + "/busco/all_stats.tsv"
	output: out_dir + "/busco/busco_stats.pdf"
	params: out_dir = out_dir
	script: "scripts/plot-busco.R"

rule plot_dnadiff:
	conda: "env/env-r.yaml"
	input: out_dir + "/dnadiff/all_stats.tsv"
	output: list_dnadiff_pdf
	params: out_dir = out_dir
	script: "scripts/plot-dnadiff.R"

rule plot_nucdiff:
	conda: "env/env-r.yaml"
	input: out_dir + "/nucdiff/all_stats.tsv"
	output: list_nucdiff_pdf
	params: out_dir = out_dir
